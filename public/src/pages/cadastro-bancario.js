(() => {
  ...
  const bankingStripSaldo = document.getElementById("banking-strip-saldo");
  const bankingStripSaldoMeta = document.getElementById("banking-strip-saldo-meta");
  const bankingStripSalario = document.getElementById("banking-strip-salario");
  const bankingStripSalarioMeta = document.getElementById("banking-strip-salario-meta");
  const bankingStripPagamento = document.getElementById("banking-strip-pagamento");
  const bankingStripPagamentoMeta = document.getElementById("banking-strip-pagamento-meta");
  const bankingStripCiclo = document.getElementById("banking-strip-ciclo");
  const bankingStripCicloMeta = document.getElementById("banking-strip-ciclo-meta");
  ...
  function isBalanceSummaryDescription(value) {
    const normalized = normalizeHeaderName(normalizeStatementDescription(value));
    return (
      isStatementSummaryDescription(value) ||
  ...
  function parseMercadoPagoCSV(textContent) {
    ...
    const headerIndex = lines.findIndex((line) => {
      ...
      return normalized.includes("RELEASE_DATE") &&
        normalized.includes("TRANSACTION_TYPE") &&
        normalized.includes("TRANSACTION_NET_AMOUNT");
    });

    if (headerIndex === -1) {
      throw new Error(
        "Nao encontrei o bloco de movimentacoes (TRANSACTION_NET_AMOUNT) do Mercado Pago no CSV enviado."
      );
    }

    const header = splitDelimitedLine(lines[headerIndex], ";").map((cell) => normalizeHeaderName(cell));
    const indexes = {
      data: header.findIndex((cell) => cell.includes("RELEASE_DATE")),
      descricao: header.findIndex((cell) => cell.includes("TRANSACTION_TYPE")),
      referencia: header.findIndex((cell) => cell.includes("REFERENCE_ID")),
      valor: header.findIndex((cell) => cell.includes("TRANSACTION_NET_AMOUNT")),
    };

    if (indexes.data === -1 || indexes.descricao === -1 || indexes.valor === -1) {
      throw new Error("O cabecalho do CSV nao possui as colunas necessarias (RELEASE_DATE, TRANSACTION_TYPE, TRANSACTION_NET_AMOUNT).");
    }
    ...
    for (let index = headerIndex + 1; index < lines.length; index += 1) {
      ...
      // Skip duplicated headers that might appear between pages/sections
      if (normalizeHeaderName(row[indexes.data] || "").includes("RELEASE_DATE")) {
        continue;
      }

      const releaseDate = String(row[indexes.data] || "").trim();
      const transactionType = normalizeStatementDescription(row[indexes.descricao]);
      const referenceId = indexes.referencia >= 0 ? String(row[indexes.referencia] || "").trim() : "";
      const transactionNetAmount = parseBrazilianAmountStrict(row[indexes.valor]);
      ...
      if (
        isBalanceSummaryDescription(transactionType) ||
      ...
      lancamentos.push(
        buildImportedStatementItem({
          ...
          balance: null, // Ignore PARTIAL_BALANCE mapping as requested
          bank: "mercado-pago",
          lineNumber: index + 1,
        })
      );
  ...
  function parseGenericCSV(textContent, bank) {
    ...
    for (let index = headerIndex + 1; index < lines.length; index += 1) {
      ...
      if (isBalanceSummaryDescription(description) || !isMeaningfulStatementDescription(description)) {
        continue;
      }
  ...
  function montarRevisaoImportacaoOcr(session) {
    ...
    if (!reviewItems.length) {
      ...
      <div class="subtle-panel">
        ...
        <span class="section-note">
          ${session.fileName || "Arquivo sem nome"} | ${session.bank || "Banco nao identificado"} | ${session.format || "formato nao identificado"
        }
        </span>
    ...
    return reviewItems
      .map(
        ...
        <div class="subtle-panel" data-statement-id="${item.id}">
          ...
          <span class="section-note">
            ${item.data || ""} | Confianca: ${Math.round(Number(item.confidence || 0) * 100)}% | ${item.tipo === "saida" ? "Saida" : "Entrada"
          }
          </span>
  ...
  function renderStatementPreview() {
    ...
    const manualBalance = Number(currentBanking?.saldoAtual || 0);
    const balanceDiff = Math.abs(manualBalance - Number(statementBalanceSuggestion || 0));
    if (currentBanking?.origemSaldo?.modo === "manual" && manualBalance > 0) {
      getElement("statement-status-chip").textContent =
        balanceDiff < 0.01 ? "Saldo bate com o manual" : `Saldo diverge: ${formatBankingCurrency(balanceDiff)}`;
    }
    ...
    return lancamentos
      .map(
        ...
        <div class="subtle-panel" data-statement-id="${item.id}">
          <strong>${item.descricao}</strong>
          <span class="section-note">
            ${item.data} | ${item.tipo === "saida" ? "Saida" : "Entrada"} | Categoria sugerida: ${item.categoria || item.categoriaSugerida || "outros"
          }
          </span>
          <div class="receipt-form-grid">
            <div class="field">
  ...
  function bindAccordionToggles() {
    if (document.body?.dataset.page === "banking") {
  ...
  function updateSummaryPreview() {
    ...
    const balanceMode = previewData?.banking?.origemSaldo?.modo === "extrato" ? "Base por extrato" : "Base manual";
    ...
    if (bankingStripSaldo) {
      ...
      bankingStripSaldoMeta.textContent = balanceMode;
      bankingStripSalario.textContent = formatBankingCurrency(summary.salarioLiquido || 0);
      bankingStripSalarioMeta.textContent =
        salarioLiquidoModoState === "manual" ? "Liquido confirmado" : "Estimativa atual";
      bankingStripPagamento.textContent = summary.paymentInfo.nextDate
        ? formatBankingDateLong(summary.paymentInfo.nextDate)
        : "--";
      bankingStripPagamentoMeta.textContent = summary.paymentInfo.value > 0
        ? formatBankingCurrency(summary.paymentInfo.value)
        : "Sem previsao";
      bankingStripCiclo.textContent = `${summary.diasRestantes} dia(s)`;
      bankingStripCicloMeta.textContent = summary.paymentInfo.configured
  ...
  function renderBenefitReceipt() {
    ...
    const active = true;
    const currentValue = getElement("vrvaValorPrevisto")?.value;
    const currentDateValue = getElement("vrvaDataPrevista")?.value;
    ...
    getElement("vrvaRecebido").checked = receipt.status === "recebido" || Boolean(receipt.valorRecebido);

    ["vrvaDataPrevista", "vrvaValorPrevisto", "vrvaRecebido"].forEach((id) => {
      getElement(id).disabled = false;
    });

    getElement("save-vrva-button").disabled = false;
    getElement("edit-vrva-button").disabled = false;
    ...
    try {
      ...
      showMessage(
        ...
        parserResult?.duplicateCount
          ...
          : "Arquivo lido com sucesso. Os lancamentos serao importados automaticamente."
      );
      ...
      showMessage(
        ...
        "success",
        parserResult?.duplicateCount
          ? `Arquivo lido e importado automaticamente. ${parserResult.duplicateCount} duplicidade(s) foram ignoradas.`
          : "Arquivo lido e importado automaticamente."
      );
    } catch (error) {
      statementDraftsState = [];
      statementBalanceSuggestion = 0;
  ...
  function confirmImportedExpenses() {
    ...
    const payloadToSave = reviewedDrafts.map((item) => ({
      ...item,
    ...
    const savedResult = window.FinanceStore.salvarContasVariaveisImportadas(payloadToSave);
    if (statementBalanceActions) {
  ...
  function editPaymentReceipt() {
    ...
    getElement("pagamentoDataPrevista").value = formatInputDate(receipt?.dataPrevista || getElement("pagamentoDataPrevista").value);
    getElement("pagamentoValorPrevisto").value = receipt?.valorPrevisto ? toBankingNumber(receipt.valorPrevisto) : "";
    getElement("pagamentoValorPrevisto").focus();
    renderPaymentReceipt();
  ...
  function editVrVaReceipt() {
    ...
    getElement("vrvaDataPrevista").value = formatInputDate(receipt?.dataPrevista || getElement("vrvaDataPrevista").value);
    getElement("vrvaValorPrevisto").value = receipt?.valorPrevisto ? toBankingNumber(receipt.valorPrevisto) : "";
    getElement("vrvaValorPrevisto").focus();
    renderBenefitReceipt();
