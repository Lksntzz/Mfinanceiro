...
const overviewHelpers = {
  ...
  getCardsSummary(data, referenceDate) {
    return getDashboardCardsSummary(data, referenceDate);
  },
...
function renderMetrics(summary, expenseOverview, data) {
...
function renderExpenseEvolution(summary) {
  if (window.DashboardDetails?.renderExpenseEvolution) {
    window.DashboardDetails.renderExpenseEvolution(summary);
  }
}
...
function renderInsights(summary, selectedSummary, expenseOverview, intelligence) {
  if (window.DashboardInsights?.renderInsights) {
    window.DashboardInsights.renderInsights(summary, selectedSummary, expenseOverview, intelligence);
  }
}

function renderDailySummary(data, periodSummary) {
  if (window.DashboardHistory?.renderDailySummary) {
    window.DashboardHistory.renderDailySummary(data, periodSummary);
  }
}
...
function bindHistoryFilters() {
  if (window.DashboardHistory?.bindHistoryFilters) {
    window.DashboardHistory.bindHistoryFilters();
  }
}
...
function atualizarDashboard() {
  ...
  renderMetrics(summary, expenseOverview, data);
...
function assignDashboardModules() {
  ...
  ({
    ...
    getCardsSummary: getDashboardCardsSummary,
...
function validateDashboardDependencies() {
  const dependencies = [
    ...
    ["DashboardDetails", window.DashboardDetails, [
      "renderExpenseEvolution",
      "renderSummaryTable",
    ]],
    ["DashboardInsights", window.DashboardInsights, [
      "renderInsights",
    ]],
    ["DashboardHistory", window.DashboardHistory, [
      "renderDailySummary",
      "bindHistoryFilters",
    ]],
  ];
